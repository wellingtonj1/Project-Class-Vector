...
int main()
{
	...
	while(continua=='s')
	{
		...
		conjvet p1(tamanhoconjunto), p2(tamanhoconjunto2);
		repete="sim";
		while(repete=="sim")
		{	
			...
			p2.lervet();
			conjvet aux(tamanhoconjunto+tamanhoconjunto2);
			system("clear");
			system("cls");
			cout<<"-----------------------------------------Trabalho de Conjuntos .By JaPaCoDeR! -----------------------------------------\n\n";
			cout<<"\nValores Digitados : Conjunto A = [ "; 
			p1.impvet(); cout<< "]";
			cout<< " Conjunto B = [ " ;
			p2.impvet() ; cout<< "]";
			cout<<"\n\n\nDigite 1 para { Descobrir se os conjuntos são vazios }\n";
			...
			cout << "\n Desejas realizar outra operação com os mesmos tamanhos já inseridos ? sim ou não ? "<<endl;
			cin>>repete;
			system("clear");
			system("cls");
